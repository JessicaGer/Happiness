...
<center>![Berlin](https://static.wixstatic.com/media/dae877_cac3c7f145ea484e95fb4a66e7efdcf8~mv2.jpg/v1/fill/w_640,h_516,al_c,q_80,usm_0.66_1.00_0.01/dae877_cac3c7f145ea484e95fb4a66e7efdcf8~mv2.webp){height="400px" width="200px"}</center>
...
ggplot() + geom_line(data=df1, aes(x=Monat, y=value, color=Temperature, group=Temperature)) +
   scale_color_brewer(palette="Dark2")+
  geom_bar(klima_DE1, mapping = aes(x=Monat, y=Sonnenstd), stat="identity", alpha=0.5, fill = "orange") +
  scale_y_continuous(
    name = "Temperature(°C)",
    sec.axis = sec_axis( trans=~.*1, name="Durchschnittliche tägliche Sonnenstunde (Std.)")
  ) +
  theme_light() + 
...
klima_DE2 %>% 
 ...
 geom_point(size = 4, alpha = 0.6) +  
  ...
  labs(title = "Niederschlag und Regentage pro Monat in Berlin", 
       caption ="Quelle:Climate-Data.Org",
       x = "Monat") + 
  geom_text(aes(label = value), size = 3, vjust = -0.7, color = "black") +
  theme_light() +   
  theme(legend.position = "bottom",
...
### Wie hoch sind die Reiseausgaben für Übernachtungen in Deutschland im Vergleich zu anderen Ländern Europas? ####

```{r message=FALSE, warning=FALSE, paged.print=FALSE, layout="l-body", fig.width=10, fig.height=6, fig.align="center"}

tourismus_col <- tourismus %>% 
mutate(mycolor = ifelse(Land == "Deutschland","#71CA5B","#00000"))

ggplot(tourismus_col, aes(x = fct_reorder(Land, expenditure), y = expenditure))+ 
  geom_point(size= 4, alpha = 0.8) +  
  geom_line()+
  geom_segment(aes(x = Land, xend= Land, y=0, yend= expenditure, color=mycolor)) +
  coord_flip() +
  theme_light() +
  theme(panel.grid.major.y = element_blank(), axis.ticks.y = element_blank(), panel.border = element_blank(), legend.position = "none") +
  geom_text(aes(label = expenditure), size = 3, hjust = -1.2, color = "black")+
